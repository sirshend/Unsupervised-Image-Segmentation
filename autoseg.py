...
parser.add_argument('--camera', metavar='cam',
                    help='camera number')
...
# load image
image_directory = '/home/sirshendu/Desktop/Dataset/' + str(args.camera) + '/JPEGImages/'
# print(image_directory)
inp = image_directory +  str(args.input) 
print(str(args.camera),str(args.input))
# sys.exit(0)
...
for batch_idx in range(args.maxIter):
    ...
    print (batch_idx, '/', args.maxIter, ':', nLabels, loss.data.item())
    if nLabels < args.minLabels + 3 and batch_idx > 30:
...
if not args.visualize:
    ...
    im_target_rgb = np.array([label_colours[ c % 100 ] for c in im_target])
    im_target_rgb = im_target_rgb.reshape( im.shape ).astype( np.uint8 )
cv2.imwrite( "output2/output_batch30_"+str(args.camera)+"_"+str(args.input), im_target_rgb )
